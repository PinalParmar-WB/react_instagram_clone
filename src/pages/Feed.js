  ...
  const fetchPosts = async (
    ...
    try {
      ...
      const currentFilterType =
        typeOverride !== null ? typeOverride : filterType;
      const currentSearchQuery =
        queryOverride !== null ? queryOverride : searchQuery;
  ...
  const handleFollowToggle = async (e, authorProfileId, authorAccountId) => {
    ...
    try {
      ...
      const response = await fetch(
        `${baseUrl}/social-media/follow/${authorAccountId}`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${token}`,
    ...
    try {
      ...
      if (res.success) {
        setPosts((prevPosts) =>
          prevPosts.map((post) =>
            post._id === postId
              ? { ...post, isBookmarked: res.data.isBookmarked }
              : post
          )
        );
      }
                ...
                >
                  ...
                  <div className="card-body">
                    <div className="d-flex justify-content-between align-items-center mb-2">
                      ...
                      <i
                        className={`fa-bookmark ${post.isBookmarked ? 'fa-solid' : 'fa-regular'}`}

...
const Followers = () => {
  ...
  const fetchFollowers = async (pageNum) => {
    ...
    try {
      ...
      const url = `${baseUrl}/social-media/follow/list/followers/${userProfile.username}?page=${pageNum}&limit=10`;
  ...
  return (
    ...
    >
      ...
      {!hasNextPage && users.length > 0 && (
        <div className="text-center text-muted small">No more followers</div>
      )}
    </div>
  );
};

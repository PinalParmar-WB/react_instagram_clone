...
const BookmarkedPosts = () => {
  ...
  return (
    <div id="bookmarks-container" className="secondary-container overflow-auto w-100" style={{ height: 'calc(100vh - 60px)' }}>
      ...
      <div className="bg-white p-4 rounded shadow-sm border-purple mb-5 d-flex justify-content-between align-items-center">
        ...
        <span className="badge bg-purple-light text-dark fs-6">{posts.length} Saved</span>
      </div>

      {/* Empty State */}
      {posts.length === 0 && !loading ? (
        <div className="text-center mt-5 p-5 bg-light rounded shadow-sm">
          <i className="fa-regular fa-bookmark mb-3 text-muted" style={{fontSize:'3rem'}}></i>
          <h4 className="text-muted">No bookmarks yet.</h4>
          <p>Save posts from the feed to see them here.</p>
          <button className="btn btn-purple-sm mt-2" onClick={() => navigate('/home')}>Go to Feed</button>
        </div>
      ) : (
              ...
              >
                ...
                <div className="card-header bg-white border-0 d-flex align-items-center pt-3 px-3">
                  <div className="profile-image" style={{ width: "35px", height: "35px", marginRight: "10px" }}>
                    <img src={post.author?.account?.avatar?.url || "https://cdn-icons-png.flaticon.com/512/149/149071.png"} alt="avatar" style={{ width: "100%", height: "100%" }} />
                  </div>
                ...
                <div style={{ height: "250px", overflow: "hidden", backgroundColor: "#f8f9fa" }} className="mt-2 position-relative">
                  {post.images && post.images.length > 0 ? (
                    ...
                    <div className="d-flex align-items-center justify-content-center h-100 px-3 text-center">
                        <p className="text-muted small">{post.content.substring(0, 100)}...</p>
                    </div>
                  )}
                </div>
                ...
                <div className="card-body">
                  ...
                  <p className="card-text text-secondary mb-2" style={{ display: "-webkit-box", WebkitLineClamp: "2", WebkitBoxOrient: "vertical", overflow: "hidden", fontSize: "0.9rem" }}>
                    {post.content}
                  </p>
                  
                  <div className="d-flex flex-wrap gap-1 mt-2">
                    {post.tags.slice(0, 3).map((tag, i) => (<span key={i} className="badge bg-purple-light text-dark fw-normal" style={{ fontSize: "0.7rem" }}>#{tag}</span>))}
                  </div>
                </div>
              </div>
            </div>
      ...
      {!hasNextPage && posts.length > 0 && <div className="text-center text-muted small py-3">End of bookmarks</div>}
    </div>
  );
};

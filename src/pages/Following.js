...
const Following = () => {
  ...
  const fetchFollowing = async (pageNum) => {
    ...
    try {
      ...
      const url = `${baseUrl}/social-media/follow/list/following/${userProfile.account.username}?page=${pageNum}&limit=10`;
  ...
  useEffect(() => { fetchFollowing(page); }, [page]);
  ...
  return (
    <div id="following-container" className="secondary-container overflow-auto w-100" style={{ height: 'calc(100vh - 60px)' }}>
      ...
      <div className="bg-white p-4 rounded shadow-sm border-purple mb-4 d-flex justify-content-between align-items-center">
        <h2 className="title mb-0" style={{ color: "var(--clr-purple)" }}>People I Follow</h2>
      ...
      {!hasNextPage && users.length > 0 && <div className="text-center text-muted small">No more following</div>}
    </div>
  );
};
